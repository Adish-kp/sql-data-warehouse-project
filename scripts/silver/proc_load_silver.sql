...
BEGIN
	...
	BEGIN TRY
		...
		PRINT '======================================';
		PRINT '----------------------------------------';
		PRINT 'Loading CRM silver.crm_cust_info';
		PRINT '----------------------------------------';
		...
		END AS sls_due_dt,
		CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quandity * ABS(sls_price)
			THEN sls_quandity * ABS(sls_price)
		ELSE sls_sales 
		END AS sls_sales,
		sls_quandity,
		CASE WHEN sls_price IS NULL OR sls_price <= 0
			THEN sls_sales / NULLIF(sls_quandity,0)
		ELSE sls_price 
		END AS sls_price
		...
		PRINT '----------------------------------------';
		PRINT '>> Truncating Table: silver.erp_cst_az1';
